# Simple PowerShell GCP Firecracker Deployment Script
param(
...
try {
    ...
    Write-Host "gcloud CLI found: $($gcloudVersion[0])" -ForegroundColor Green
} catch {
...
Write-Host "Creating startup script file..." -ForegroundColor Yellow

# Create startup script as separate file to avoid PowerShell parsing issues
$bashScript = @'
#!/bin/bash
set -e
...
$bashScript | Out-File -FilePath "startup-script.sh" -Encoding UTF8
...
Write-Host "Creating GCP VM..." -ForegroundColor Yellow
...
# Create the GCP VM
Write-Host "Creating VM with nested virtualization..." -ForegroundColor Blue
...
Write-Host "VM is starting up. Monitoring deployment..." -ForegroundColor Blue
Write-Host "This will take 5-10 minutes. You can also monitor manually with:" -ForegroundColor Blue
...
for ($i = 1; $i -le $maxAttempts; $i++) {
    ...
    try {
        $result = gcloud compute ssh $VmName --zone=$Zone --command="test -f /tmp/deployment-complete" --quiet 2>$null
        if ($LASTEXITCODE -eq 0) {
...
if ($deploymentComplete) {
    ...
    Write-Host "Deployment may still be in progress. Check manually if needed." -ForegroundColor Yellow
}
...
Write-Host "  Check Firecracker: sudo firecracker --version"
Write-Host "  View setup: ls -la /opt/firecracker/"
Write-Host ""
...
try {
    $response = Invoke-WebRequest -Uri "http://$externalIp" -TimeoutSec 10 -UseBasicParsing -ErrorAction Stop
...
# Clean up
Remove-Item -Path "startup-script.sh" -Force -ErrorAction SilentlyContinue

Write-Host ""
Write-Host "🎯 Next Steps:" -ForegroundColor Green
Write-Host "1. Visit your application: http://$externalIp"
Write-Host "2. SSH into VM: gcloud compute ssh $VmName --zone=$Zone"
Write-Host "3. Test Firecracker: sudo firecracker --version"
Write-Host "4. Check setup: ls -la /opt/firecracker/"
Write-Host ""
